...
const Map: NextPage = ({ mapData, startCity, endCity }: InferGetStaticPropsType<typeof getStaticProps>) => {
  ...
  const [currentPoint, setCurrentPoint] = useState<CityPoint>({...convertToRelScreenCoords(mapData, startCity.lat, startCity.lng),
        name: startCity.name})
  const [endPoint, setEndPoint] = useState<CityPoint>({...convertToRelScreenCoords(mapData, endCity.lat, endCity.lng),
        name: endCity.name})

  // Multiply search radius by modifier when searchRadius is changed
  const [searchRadius, setSearchRadius] = useState<number>(mapData.searchRadius)
      ...
      .then((cities) => {
        ...
        cities
          .map((city): CityPoint => {
            ...
            return {...convertToRelScreenCoords(mapData, city.lat, city.lng), name: city.name }
          })
          .forEach((city, i) => {
        ...
        if (endPointCandidate) {
          if (withinRange(endPoint.y * svgRef.current!!.clientHeight, endPoint.x * svgRef.current!!.clientWidth,
            currentPoint!!.y * svgRef.current!!.clientHeight, currentPoint!!.x * svgRef.current!!.clientWidth, searchRadius)) {
        ...
        // Is within circle?
        if (withinRange(closestCity.y * svgRef.current!!.clientHeight, closestCity.x * svgRef.current!!.clientWidth,
          currentPoint!!.y * svgRef.current!!.clientHeight, currentPoint!!.x * svgRef.current!!.clientWidth, searchRadius)) {
...
export const getStaticProps: GetStaticProps = async (context) => {
  ...
  const response: GeoResponse = await getRandomCities(mapData).then((value) => value.json())
